...
async function refreshThread() {
  try {
    ...
    const response = await fetch(
      "https://getthreads-pgktbhms6a-uc.a.run.app/",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (threadData) {
      ...
      document.getElementById(
        "threadAuthor"
      ).innerText = `Posted by ${threadData.author} on ${threadData.timestamp}`;
    ...
    comments.forEach((comment) => {
      ...
      li.innerHTML = `
        ...
        comment.timestamp
                ...
                <button id="deleteCommentButton" style="border-radius:25px;"onclick="deleteComment(${
                  comment.id
...
document
  .getElementById("deleteThreadButton")
